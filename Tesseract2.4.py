...
img = Image.open('images/Crop1.jpg')
...
for i in range(1, nb_components):
    ...
    extractedText=extractText(crop_img)

    try:
        translatedText=translator.translate(extractedText,dest='ro')
    ...
    wrapped_text = textwrap.wrap(translatedText, width=80)
    ...
    if(len(wrapped_text)>=1):
        ...
        while(textsize[0]<stats[i,cv2.CC_STAT_WIDTH]*0.8):
            font_size+=0.1
            textsize = cv2.getTextSize(wrapped_text[indexMax], cv2.FONT_HERSHEY_COMPLEX, font_size, font_thickness)[0]

        gap = int(textsize[1]/3)
        y=0
        for j, line in enumerate(wrapped_text):
            #---------------------------------------------------------------------------------------------------------
            #---------------------------------------------------------------------------------------------------------
            #print('---->'+line)
            #textsize = cv2.getTextSize(line, cv2.FONT_HERSHEY_COMPLEX, 1, font_thickness)[0]
            
            #font_size=stats[i,cv2.CC_STAT_HEIGHT]/(len(wrapped_text)*textsize[0])

            y = int((stats[i,cv2.CC_STAT_TOP] + textsize[1]*(j+1))) + gap
            x = int(stats[i,cv2.CC_STAT_LEFT] + stats[i,cv2.CC_STAT_WIDTH]*0.1 )
            print(str(j)+" "+line +"la "+str(x)+" "+str(y))
            cv2.putText(deskewed,
            line,
            (x, y),
